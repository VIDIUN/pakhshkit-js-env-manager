  ...
  runReleaseMode() {
    return new Promise((resolve) => {
      ...
      Utils.showPrompt('Observe upcoming release. Do you want to continue?', (ans) => {
        if (ans) {
          updatePakhshkitJsDependenciesVersions.call(this);
          ...
          ReleaseDataHandler.set(this.name, {
            url: '//github.com/vidiun/' + this.name + '/releases/tag/v' + newVersion,
            version: newVersion,
            changeLog: changelog
          });
...
function updatePakhshkitJsDependenciesVersions() {
  let updated = false;
...
module.exports = PakhshkitRepoController;
